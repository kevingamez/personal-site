...
export async function Sidebar() {
  ...
  return (
    <aside className="sidebar">
      <div className="panel">
        <div className="ph"><b>github · @kevingamez</b><span className="pos">live</span></div>
        <div className="row2">
          <div className="metric">
            <div className="vl">{stats.publicRepos}<span> repos</span></div>
            <div className="sub">{stats.followers} followers · {stats.following} following</div>
          </div>
          <div className="metric">
            <div className="vl">{stats.yearsOnGithub}<span> yrs</span></div>
            <div className="sub">{joinedLabel}</div>
          </div>
        </div>
        <div className="spark" id="spark" aria-label="Language mix across public repos">
          {langBars.map((l, i) => (
            <span key={i} style={{ height: `${l.height}%`, background: l.color }} title={`${l.name} ${l.pct}%`} />
          ))}
        </div>
      </div>

      <div className="panel">
        <div className="ph"><b>in flight</b><span className="pos">2026</span></div>
        <div className="flight-list">
          ...
          >
            ...
            <div className="fl-body">
              <div className="fl-title">ai outbound · <span className="fl-where">enttor</span></div>
          ...
          >
            ...
            <div className="fl-body">
              <div className="fl-title">this site · <span className="fl-where">open source</span></div>
          ...
          >
            ...
            <div className="fl-body">
              <div className="fl-title">AD_ASTRA · <span className="fl-where">m.sc. thesis</span></div>
      ...
      <div className="panel">
        <div className="ph"><b>tail -f /var/log/commits</b><span className="pos">github · live</span></div>
        <div className="reqs commit-tail" id="reqs">
          ...
          {recentCommits.map((c, i) => (
            <a key={i} className="r commit-r" href={c.url} target="_blank" rel="noopener">
              <span className="ts">{tailTime(c.ts)}</span> <span className="sha">{c.shaShort}</span>{" "}
              <span className="pa">
                <span className="ct-repo">{c.repoShort}</span> <span className="ct-msg">{c.message}</span>
              </span>
            </a>
          ))}
        </div>
      ...
      <div className="panel">
        <div className="ph"><b>contributions · 12mo</b><span className="pos">github</span></div>
        <div className="hm" id="hm" data-contrib={JSON.stringify(stats.contribCalendar.days.map((d) => d.level))}></div>
        <div className="hm-legend">
          <span>12mo ago</span> <span className="hm-total">{stats.contribCalendar.totalContributions.toLocaleString()} contributions</span>{" "}
          <span>today</span>
        </div>
      </div>

      <div className="panel">
        <div className="ph"><b>open source</b><span className="pos">@kevingamez</span></div>
        <div className="repos">
          {featured.map((r, i) => (
      ...
      <div className="panel">
        <div className="ph"><b>now() →</b></div>
        <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "11.5px", lineHeight: 1.7, color: "var(--ink-2)" }}>
          <div><span className="ac">building</span> · ai outbound @ enttor</div>
          <div><span className="ac">stack</span> · next.js + nestjs + postgres</div>
          <div><span className="ac">studying</span> · deep learning, browser automation</div>
          <div><span className="ac">based</span> · bogotá, co · utc-5</div>
        </div>
      </div>
    </aside>
  )
